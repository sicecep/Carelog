...
import { ArchiveBox, ArrowClockwise } from "phosphor-react";
import { recipientApi } from "@/lib/api-client";

interface RecipientArchiveActionsProps {
  recipientId: string;
  workspaceId: string;
  isActive: boolean;
  onRefresh: () => void;
...
export function RecipientArchiveActions({ recipientId, workspaceId, isActive, onRefresh }: RecipientArchiveActionsProps) {
    ...
    <div className="flex gap-2">
      {isActive ? (
        <button
          ...
          disabled={busy}
          className="btn-base btn-ghost btn-icon touch-target text-[var(--color-text-muted)] hover:text-[var(--color-error-ink)]"
          title={t("archive")}
        >
          <ArchiveBox size={20} />
        </button>
